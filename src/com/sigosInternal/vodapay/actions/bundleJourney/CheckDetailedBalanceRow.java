...
public class CheckDetailedBalanceRow extends Action
{
	...
	protected ScriptReturn run(Device device, IScriptContext context) throws Exception
	{
		...
		if (!ScreenSync.waitFor(device, getCurrentContext(), SCREEN_TRANSITION_TIMEOUT_MS, summaryCardLabel))
		...
		if (!ScreenSync.waitFor(device, getCurrentContext(), SCREEN_TRANSITION_TIMEOUT_MS, DETAILED_BALANCES_MENU_LABEL))
		...
		if (!ScreenSync.waitFor(device, getCurrentContext(), SCREEN_TRANSITION_TIMEOUT_MS, LEFT_MARKER))
